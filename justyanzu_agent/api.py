...
_redis_broker = os.getenv("REDIS_URL") or os.getenv("REDIS_BASE_URL") or "redis://localhost:6379/0"
celery_app = Celery("tasks", broker=_redis_broker, backend=_redis_broker)
...
def initialize_system():
    ...
    def _api_daily_summary_kwargs(cfg):
        sec = "MAIN"
        return {
    ...
    interaction = Interaction(
        ...
        langs=languages,
        **_api_daily_summary_kwargs(config),
    )
...
async def process_query(request: QueryRequest):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"error": str(e), "done": "false"},
        )
    finally:
