...
class searxSearch(Tools):
    def __init__(self, base_url: str = None):
        ...
        未配置 SEARXNG_BASE_URL 时不抛错，execute 会返回禁用说明（便于在无 SearxNG 的部署下启动）。
        """
        super().__init__()
        self.tag = "web_search"
        ...
        if base_url:
            self.base_url = base_url.strip().rstrip("/") or None
        else:
            env = os.getenv("SEARXNG_BASE_URL", "").strip().rstrip("/")
            self.base_url = env if env else None
    ...
    def execute(self, blocks: list, safety: bool = False) -> str:
        ...
        if not self.base_url:
            return (
                "Error: SearxNG 未配置（未设置环境变量 SEARXNG_BASE_URL）。"
                "Browser Agent 的网页检索不可用；请自建 SearxNG 并配置该变量，或改用其它联网方式。"
            )
