...
class Provider:
    def __init__(self, provider_name, model, server_address="127.0.0.1:5000", is_local=False):
        ...
        self.unsafe_providers = ["openai", "deepseek", "dsk_deepseek", "together", "google", "openrouter", "kimi"]
    ...
    def get_api_key(self, provider):
        load_dotenv()
        # KIMI/Moonshot 支持 MOONSHOT_API_KEY 或 KIMI_API_KEY
        if provider.lower() == "kimi":
            api_key = os.getenv("MOONSHOT_API_KEY") or os.getenv("KIMI_API_KEY")
            api_key_var = "MOONSHOT_API_KEY 或 KIMI_API_KEY"
            self.api_key_source = "MOONSHOT_API_KEY" if os.getenv("MOONSHOT_API_KEY") else ("KIMI_API_KEY" if os.getenv("KIMI_API_KEY") else None)
        else:
            api_key_var = f"{provider.upper()}_API_KEY"
            api_key = os.getenv(api_key_var)
            self.api_key_source = api_key_var
    ...
    def kimi_fn(self, history, verbose=False):
        """
        Use Moonshot AI (KIMI) API to generate text.
        官方文档: https://platform.moonshot.ai
        模型示例: kimi-k2-thinking, kimi-k2-turbo-preview
        国内用户可用 api.moonshot.cn，国际用户可用 api.moonshot.ai
        """
        # 支持通过 KIMI_BASE_URL 覆盖，默认用 api.moonshot.ai（官方文档）
        ...
        if self.is_local:
            raise Exception("KIMI (API) is not available for local use. Change config.ini")
        try:
            response = client.chat.completions.create(
                model=self.model,
                messages=history,
            ...
            if response is None:
                raise Exception("KIMI response is empty.")
            thought = response.choices[0].message.content
            if verbose:
                print(thought)
            return thought
            ...
            err_msg = str(e)
            if "401" in err_msg or "Invalid Authentication" in err_msg or "invalid_authentication" in err_msg:
                key = self.api_key or ""
                key_preview = (key[:6] + "..." + key[-4:]) if len(key) >= 12 else (key[:3] + "...") if key else "EMPTY"
                raise Exception(
                    f"KIMI API 401 认证失败。请检查：\n"
                    f"  0. 当前调用 base_url={base_url}\n"
                    f"  0. 当前读取的环境变量={self.api_key_source}，key 形态={key_preview}，长度={len(key) if key else 0}\n"
                    f"  1. .env 里必须填“实际密钥”，不要填 $MOONSHOT_API_KEY 这种引用；密钥通常以 sk- 开头\n"
                    f"  2. 你提到是在“KIMI Code 控制台”生成的 key：请确认它是“Open Platform / Kimi API”可用的 API Key（控制台地址通常是 platform.moonshot.ai 的 API Keys）。有些产品侧的 key 不能用于 OpenAI 兼容 API，会直接 401\n"
                    f"  3. 国内用户可尝试在 .env 添加 KIMI_BASE_URL=https://api.moonshot.cn/v1（与 key 来源保持一致）"
                ) from e
            raise Exception(f"KIMI API error: {err_msg}") from e
